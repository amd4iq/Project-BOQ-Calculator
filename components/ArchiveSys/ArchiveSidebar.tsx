...
// Compact Filter Input for tighter layout
...
export const ArchiveSidebar: React.FC<ArchiveSidebarProps> = ({
    ...
    const [isSearchExpanded, setIsSearchExpanded] = useState(false); // Closed by default
        ...
        <aside className="w-80 bg-white border-l border-slate-200 flex flex-col h-full shadow-lg print:hidden z-50 relative">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
                ...
                <nav className="space-y-2">
                    ...
                    <div className="px-2 pb-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">السجلات</div>
                    <NavButton 
                        icon="file-text"
                ...
                {/* Advanced Search - Compact Design */}
                <div className="border-t border-slate-100 pt-4">
                    <button 
                        onClick={() => setIsSearchExpanded(!isSearchExpanded)}
                        className={`w-full flex items-center justify-between p-3 rounded-xl transition-colors ${isSearchExpanded ? 'bg-slate-100 text-slate-800' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'}`}
                    >
                        <div className="flex items-center gap-2">
                            <Icon name="search" size={18} />
                            <h3 className="font-bold text-sm">أدوات البحث والفلترة</h3>
                        </div>
                    ...
                    {isSearchExpanded && (
                        <div className="p-3 bg-slate-50 rounded-xl mt-2 border border-slate-200 space-y-3 animate-in slide-in-from-top-2 duration-200">
                            ...
                            >
                                ...
                                <option value="Contract Archived">مؤرشف</option>
                            ...
                            <button 
                                ...
                                className="w-full text-center text-[10px] font-bold text-slate-500 hover:text-red-600 py-1.5 rounded hover:bg-red-50 transition-colors border border-dashed border-slate-300 hover:border-red-200"
                            >
                                إعادة تعيين الفلاتر
                            </button>
                        </div>
                        ...
                        >
                            <div className="relative bg-transparent hover:bg-white/10 transition-colors rounded-[10px] h-full flex items-center justify-center px-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-bold text-xs whitespace-nowrap">إدارة العقود والمشاريع</span>
                                </div>
                            </div>
                        </button>
                    )}
