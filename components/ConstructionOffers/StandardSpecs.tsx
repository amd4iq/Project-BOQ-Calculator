...
export const StandardSpecs: React.FC<StandardSpecsProps> = ({ specs, onAdd, onDelete, onUpdate, isReadOnly }) => {
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full animate-in fade-in zoom-in-95 duration-300 ${isReadOnly ? 'opacity-70 bg-slate-50' : ''}`}>
        ...
        <table className="w-full text-sm text-right">
          <thead>
            <tr className="border-b border-slate-100 text-slate-500 bg-slate-50/30">
              ...
              {!isReadOnly && <th className="px-4 py-3 w-24 text-center">إجراءات</th>}
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-50">
              ...
              <tr className="bg-slate-50/80 border-b border-slate-100 shadow-inner">
                  ...
                  <td className="px-4 py-3 text-center">
                      <button
                          ...
                          disabled={!quickText.trim() || isReadOnly}
            ...
            {specs.length === 0 && (
               <tr>
                 <td colSpan={!isReadOnly ? 4 : 3} className="px-6 py-12 text-center text-slate-400">
